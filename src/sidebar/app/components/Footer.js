...
class Footer extends React.Component {
    ...
    switch (eventData.action) {
      ...
      case 'text-saved':
        if (! this.state.waitingToReconnect) {
          ...
          this.setState({
            savingIndicatorText: browser.i18n.getMessage('savedComplete2', formatFooterTime())
          });
        }
        // Enable sync-action
        this.setState({
  ...
  getLastSyncedTime = () => {
    ...
    if (this.state.isAuthenticated) {
      ...
      this.setState({
        savingIndicatorText: browser.i18n.getMessage('syncComplete2', formatFooterTime(this.state.lastModified)),
        isAuthenticated: true
      });
      ...
      this.setState({
        savingIndicatorText: browser.i18n.getMessage('savedComplete2', formatFooterTime(this.state.lastModified))
      });
    }
  };
  ...
  enableSyncAction = (editor) => {
    ...
    if (this.state.isAuthenticated && footerButtons.classList.contains('syncingLayout')) {
      // Trigger manual sync
      setAnimation(true);
      browser.runtime.sendMessage({
          action: 'kinto-sync'
        });
    } else if (!this.state.isAuthenticated && (footerButtons.classList.contains('savingLayout') || this.state.waitingToReconnect)) {
      // Login
      setAnimation(true, true, false);  // animateSyncIcon, syncingLayout, warning
      ...
      <footer>
        ...
        <div id="footer-buttons" className="savingLayout">
          <div>
            ...
            <button id="enable-sync" onClick={this.enableSyncAction} className="notsyncing">
          ...
          <a id="give-feedback-button" title={browser.i18n.getMessage('feedback')} href={SURVEY_PATH}>
            <svg id="Layer_1" width="20" height="20" data-name="Layer 1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16">
              <defs><style>.cls-1{'{'}fill:none;stroke:#333;stroke-miterlimit:10{'}'} .cls-2{'{'}fill:#333{'}'}</style></defs>
              <path className="cls-1" d="M14.32 10.58A5.27 5.27 0 0 0 15 8c0-3.31-3.13-6-7-6S1 4.69 1 8s3.13 6 7 6a7.72 7.72 0 0 0 4.37-1.32L15 14z"/><circle className="cls-2" cx="8" cy="8" r="1"/><circle className="cls-2" cx="4" cy="8" r="1"/><circle className="cls-2" cx="12" cy="8" r="1"/>
            </svg>
          </a>
          <div className="wrapper">
            <button id="context-menu-button" className="mdl-js-button"></button>
            <ul className="mdl-menu mdl-menu--top-right mdl-js-menu context-menu" data-mdl-for="context-menu-button">
              <li id="disconnect-from-sync" onClick={this.disconnectFromSync} className="mdl-menu__item context-menu-item"></li>
              <li><a id="give-feedback" className="mdl-menu__item context-menu-item" title={browser.i18n.getMessage('feedback')} href={SURVEY_PATH}></a></li>
            </ul>
          </div>
        </div>
      </footer>
