...
const AUTH_PARAMS = `client_id=${CLIENT_ID}
&state=state
...
function handleAuthentication() {
  ...
  return createKeyPair()
    ...
    .then(function(publicKey) {
      return getKintoFxAConfig(KINTO_URL).then(function(params) {
        oauth_url = params.oauth_uri;
        const scope = params.scope;
        return browser.identity.launchWebAuthFlow({
          interactive: true,
          url:
            `${oauth_url}/authorization?` +
    ...
    .then(function(decrypted) {
      ...
      const decryptedKeys = JSON.parse(
        new TextDecoder('utf-8').decode(new Uint8Array(filtered))
      );
      console.log('decryptedKeys', decryptedKeys);

      chrome.storage.local.set(
        { bearer: bearerToken, keys: decryptedKeys },
        function() {
